...
export default function Leaderboard() {
  ...
  const players = useMemo(() => {
    ...
    const all = [...bots, ...users].sort((a, b) => b.elo - a.elo);
    return all;
  }, [currentUser]);

  return (
    <div style={commonStyles.page}>
      <h1 style={{ color: colors.text, marginBottom: spacing.lg }}>Leaderboard</h1>
      <div style={commonStyles.card}>
        <div style={{ display: 'flex', padding: '12px 16px', borderBottom: `2px solid ${colors.border}`, color: colors.textMuted, fontSize: 13, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1 }}>
          ...
          return (
            <div
              ...
              style={{
                ...
                backgroundColor: isCurrentUser ? colors.accentLight : idx % 2 === 0 ? 'transparent' : colors.bgDeep,
              }}
            >
              <div style={{ width: 50, fontWeight: 700, color: idx < 3 ? medalColors[idx] : colors.textMuted, fontSize: idx < 3 ? 18 : 14 }}>
                {idx + 1}
              </div>
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: spacing.sm }}>
                <Avatar username={player.username} size={32} />
                <Link to={`/profile/${player.id}`} style={{ color: colors.text, textDecoration: 'none', fontWeight: 500, fontSize: 15 }}>
                  {player.username}
                </Link>
                <Badge text={tier.name} color={tier.color} style={{ fontSize: 10 }} />
                {isCurrentUser && <span style={{ fontSize: 11, color: colors.accent, fontWeight: 600 }}>YOU</span>}
              </div>
